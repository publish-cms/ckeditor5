...
export default class BlockToolbar extends Plugin {
	...
	public init(): void {
		...
		this.listenTo<DocumentSelectionChangeRangeEvent>(
		...
		this.listenTo<ObservableChangeEvent>(
			editor,
			'change:isReadOnly',
			() => this._updateButton(),
			{ priority: 'low' }
		...
		this.listenTo<ObservableChangeEvent<boolean>>(
			this.buttonView,
			'change:isVisible',
			( evt, name, isVisible ) => {
				if ( isVisible ) {
				...
				if ( element && element.className.includes( 'ck-toolbar__separator' ) ) {
					...
					while ( !b ) {
						...
						if (
							...
							elementNext.className.includes(
